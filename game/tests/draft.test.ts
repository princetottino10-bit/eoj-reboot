...
describe('createDraftState', () => {
  ...
  it('全6派閥が available', () => {
    ...
    expect(draft.availableFactions.sort()).toEqual(
      ['aggro', 'control', 'snipe', 'synergy', 'tank', 'trick'],
    );
  });
...
describe('currentPickType', () => {
  it('step 0〜3: faction', () => {
    for (let s = 0; s <= 3; s++) {
      expect(currentPickType({ ...createDraftState(), step: s })).toBe('faction');
    }
  });
  it('step 4〜5: itemSet', () => {
    for (let s = 4; s <= 5; s++) {
      expect(currentPickType({ ...createDraftState(), step: s })).toBe('itemSet');
    }
  });
});
...
describe('buildDeck', () => {
  ...
  it('他の派閥のカードは含まれない', () => {
    const deck = buildDeck(['aggro', 'tank'], 'A', MOCK_CARDS);
    const others = deck.filter(id =>
      !id.startsWith('aggro') && !id.startsWith('tank') && !id.startsWith('item'),
    );
