...
describe("getValidSummonCells", () => {
  ...
  it("味方が中央(4)にいるとき: 隣接する空きマスのみ", () => {
    ...
    expect(getValidSummonCells(board, 0).sort((a, b) => a - b)).toEqual([
      1, 3, 5, 7,
    ]);
  });
...
describe("getFrontRowCells", () => {
  ...
  it("DOWN向き(dir=2): casterIdx=4 の前列(row=2)に敵がいれば返す", () => {
    ...
    board[6] = makeChar(1, {});
    board[7] = makeChar(1, {});
    const result = getFrontRowCells(board, 4, DIR_DOWN, 1).sort(
      (a, b) => a - b,
    );
  ...
  it("RIGHT向き(dir=1): casterIdx=4 の前列(col=2)に敵がいれば返す", () => {
    ...
    board[2] = makeChar(1, {});
    board[5] = makeChar(1, {});
    const result = getFrontRowCells(board, 4, DIR_RIGHT, 1).sort(
      (a, b) => a - b,
    );
  ...
  it("LEFT向き(dir=3): casterIdx=4 の前列(col=0)に敵がいれば返す", () => {
    ...
    board[0] = makeChar(1, {});
    board[3] = makeChar(1, {});
    const result = getFrontRowCells(board, 4, DIR_LEFT, 1).sort(
      (a, b) => a - b,
    );
