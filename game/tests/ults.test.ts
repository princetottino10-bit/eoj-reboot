...
import { applyUltDirectEffect, applyUltTargetEffect } from '../src/engine/ults.js';
import type { GameState, CharInstance, PlayerState, Board } from '../src/engine/types.js';
import { DIR_UP, DIR_DOWN, DIR_RIGHT, DIR_LEFT } from '../src/engine/board.js';

// ============================================================
// テストヘルパー
// ============================================================

function makeChar(owner: 0 | 1, cardId: string, opts: Partial<CharInstance> = {}): CharInstance {
  return {
    cardId, owner,
...
describe('applyUltDirectEffect: aggro_v2_12 前列5ダメ+HP1設定', () => {
  ...
  it('前列の敵を撃破した場合VP獲得とclearAffiliatedEffects', () => {
    ...
    // 洗脳付与者として登録
    board[7] = makeChar(0, 'victim', { status: { brainwashedTurns: 2, brainwashedBy: 'control_v2_01', actionTax: 0, actionTaxBy: null, dirLocked: 0, immune: 0 } });
...
describe('applyUltDirectEffect: synergy_v2_10 全味方永続バフ', () => {
  it('全味方のATK+1・最大HP+1（永続）', () => {
    const board: Board = Array(9).fill(null);
    board[4] = makeChar(0, 'synergy_v2_10', { atk: 2, baseAtk: 2, hp: 3, maxHp: 3 });
...
describe('applyUltTargetEffect: snipe_v2_09 4ダメ+clearAffiliatedEffects', () => {
  it('敵に4ダメージ（貫通）', () => {
    ...
    const state = makeState(board);
    const { newState, continueToDir } = applyUltTargetEffect(state, 4, 'snipe_v2_09', 1, 0);
...
describe('applyUltTargetEffect: snipe_v2_12 5ダメ', () => {
  it('敵に5ダメージ（貫通）', () => {
    ...
    board[1] = makeChar(1, 'enemy', { hp: 7 });
    const state = makeState(board);
    const { newState, continueToDir } = applyUltTargetEffect(state, 4, 'snipe_v2_12', 1, 0);
...
describe('applyUltTargetEffect: control_v2_09 ATK奪取', () => {
  it('敵のATKを最大2奪う（casterに付与）', () => {
    ...
    const state = makeState(board);
    const { newState, continueToDir } = applyUltTargetEffect(state, 4, 'control_v2_09', 1, 0);
...
describe('applyUltTargetEffect: trick_v2_09 位置交換+continueToDir', () => {
  it('casterと対象の位置を入れ替え、continueToDir=trueを返す', () => {
    ...
    board[1] = makeChar(1, 'enemy');
    const state = makeState(board);
    const { newState, continueToDir } = applyUltTargetEffect(state, 4, 'trick_v2_09', 1, 0);
