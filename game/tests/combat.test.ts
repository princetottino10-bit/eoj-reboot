...
import { DIR_DOWN, DIR_RIGHT, DIR_UP } from "../src/engine/board.js";
import {
  calcDamage,
  canCounterAttack,
  resolveAttack,
...
function makeChar(
  overrides: Partial<CharInstance> & { owner: 0 | 1 },
): CharInstance {
  return {
    cardId: "test",
    hp: 3,
...
describe("calcDamage", () => {
  it("基本ダメージ: ATKがそのまま", () => {
    ...
    expect(
      calcDamage(attacker, defender, { isBlind: false, teamDR: false }),
    ).toBe(3);
  });
  ...
  it("チームDR: ダメージ-1", () => {
    ...
    const defender = makeChar({ owner: 1 });
    expect(
      calcDamage(attacker, defender, { isBlind: false, teamDR: true }),
    ).toBe(2);
  });
  ...
  it("貫通マーカー: 防護を無視", () => {
    const attacker = makeChar({
      owner: 0,
      atk: 2,
      markers: { protection: 0, evasion: 0, piercing: 1, quickness: 0, aim: 0 },
    });
    const defender = makeChar({ owner: 1, keywords: ["防護"] });
    expect(
      calcDamage(attacker, defender, { isBlind: false, teamDR: false }),
