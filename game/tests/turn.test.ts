...
describe("drawStep", () => {
  ...
  it("相手のマナは変わらない", () => {
    const state = makeState({
      players: [makePlayer({ mana: 3 }), makePlayer({ mana: 2 })],
    });
    const next = drawStep(state);
  ...
  it("デッキのトップカードを手札に加える", () => {
    const state = makeState({
      players: [
        makePlayer({ deck: ["card_a", "card_b", "card_top"] }),
        makePlayer(),
      ],
    });
    const next = drawStep(state);
  ...
  it("マナ0からでも正しく+2", () => {
    const state = makeState({
      players: [makePlayer({ mana: 0 }), makePlayer()],
    });
    const next = drawStep(state);
...
describe("spendReactivationMana: マナ消費", () => {
  it("baseCost 分のマナを消費する（actionTax=0）", () => {
    ...
    board[0] = makeChar(0);
    const state = makeState({
      players: [makePlayer({ mana: 5 }), makePlayer()],
      board,
    });
  ...
  it("actionTax=2 の場合も正しく計算", () => {
    ...
    const state = makeState({
      players: [makePlayer({ mana: 6 }), makePlayer()],
      board,
    });
  ...
  it("相手のマナは変わらない", () => {
    ...
    board[0] = makeChar(0);
    const state = makeState({
      players: [makePlayer({ mana: 5 }), makePlayer({ mana: 3 })],
      board,
    });
    const next = spendReactivationMana(state, 0, 1);
...
// skipNextTurn
// ============================================================
describe("startTurnPhase: skipNextTurn", () => {
  it("skipNextTurn=1 のキャラはターン開始時に hasActed=true になる", () => {
    const board = Array(9).fill(null);
    board[0] = makeChar(0, { status: { brainwashedTurns: 0, brainwashedBy: null, actionTax: 0, actionTaxBy: null, dirLocked: 0, immune: 0, skipNextTurn: 1 } });
    const state = makeState({ active: 0, board });
    const next = startTurnPhase(state);
    expect(next.board[0]?.hasActed).toBe(true);
    expect(next.board[0]?.hasRotated).toBe(true);
  });

  it("skipNextTurn=1 は消費後に 0 になる", () => {
    const board = Array(9).fill(null);
    board[0] = makeChar(0, { status: { brainwashedTurns: 0, brainwashedBy: null, actionTax: 0, actionTaxBy: null, dirLocked: 0, immune: 0, skipNextTurn: 1 } });
    const state = makeState({ active: 0, board });
    const next = startTurnPhase(state);
    expect(next.board[0]?.status.skipNextTurn).toBe(0);
  });

  it("skipNextTurn=0 のキャラは通常通り行動可能", () => {
    const board = Array(9).fill(null);
    board[0] = makeChar(0, { hasActed: true });
    const state = makeState({ active: 0, board });
    const next = startTurnPhase(state);
  ...
  it("相手キャラの skipNextTurn は自分のターンには影響しない", () => {
    const board = Array(9).fill(null);
    board[0] = makeChar(1, { status: { brainwashedTurns: 0, brainwashedBy: null, actionTax: 0, actionTaxBy: null, dirLocked: 0, immune: 0, skipNextTurn: 1 } });
    const state = makeState({ active: 0, board });
    const next = startTurnPhase(state);
    expect(next.board[0]?.hasActed).toBe(false);
  });
