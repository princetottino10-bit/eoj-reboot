...
describe('startTurnPhase: ステータス効果のtick（現プレイヤーのキャラのみ）', () => {
  it('洗脳ターン数を1減算', () => {
    const board = Array(9).fill(null);
    board[0] = makeChar(0, {
      status: { brainwashedTurns: 2, brainwashedBy: 'ctrl_v2_01', actionTax: 0, dirLocked: 0, immune: 0 },
    });
    const next = startTurnPhase(makeState({ board, active: 0 }));
  ...
  it('洗脳が1→0になったら brainwashedBy をクリア', () => {
    const board = Array(9).fill(null);
    board[0] = makeChar(0, {
      status: { brainwashedTurns: 1, brainwashedBy: 'ctrl_v2_01', actionTax: 0, dirLocked: 0, immune: 0 },
    });
    const next = startTurnPhase(makeState({ board, active: 0 }));
  ...
  it('向き固定ターン数を1減算', () => {
    const board = Array(9).fill(null);
    board[0] = makeChar(0, {
      status: { brainwashedTurns: 0, brainwashedBy: null, actionTax: 0, dirLocked: 3, immune: 0 },
    });
    const next = startTurnPhase(makeState({ board, active: 0 }));
  ...
  it('immune ターン数を1減算', () => {
    const board = Array(9).fill(null);
    board[0] = makeChar(0, {
      status: { brainwashedTurns: 0, brainwashedBy: null, actionTax: 0, dirLocked: 0, immune: 2 },
    });
    const next = startTurnPhase(makeState({ board, active: 0 }));
  ...
  it('相手のキャラのステータスはtickしない', () => {
    const board = Array(9).fill(null);
    board[1] = makeChar(1, {
      status: { brainwashedTurns: 3, brainwashedBy: 'ctrl_v2_01', actionTax: 0, dirLocked: 2, immune: 1 },
    });
    const next = startTurnPhase(makeState({ board, active: 0 }));
