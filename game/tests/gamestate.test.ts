...
const MOCK_DB: CardDatabase = {
  characters: [
    ...Array.from({ length: 12 }, (_, i) => makeCharDef({
  ...
  items: [
    { id: 'item_01', name: 'テストアイテム1', cost: 1, vp: 1, effect: '', sets: ['A', 'B', 'C', 'D'] },
    { id: 'item_02', name: 'テストアイテム2', cost: 1, vp: 1, effect: '', sets: ['A', 'B', 'C', 'D'] },
    { id: 'item_03', name: 'テストアイテム3', cost: 1, vp: 1, effect: '', sets: ['D'] },
    { id: 'item_04', name: 'テストアイテム4', cost: 1, vp: 1, effect: '', sets: ['A', 'B'] },
    { id: 'item_05', name: 'テストアイテム5', cost: 1, vp: 1, effect: '', sets: ['B', 'D'] },
    { id: 'item_06', name: 'テストアイテム6', cost: 1, vp: 1, effect: '', sets: ['A', 'C'] },
    { id: 'item_14', name: 'テストアイテム7', cost: 1, vp: 1, effect: '', sets: ['C'] },
    { id: 'item_20', name: 'テストアイテム8', cost: 1, vp: 1, effect: '', sets: ['A'] },
    { id: 'item_bounce', name: 'テストアイテム9', cost: 1, vp: 1, effect: '', sets: ['C'] },
    { id: 'item_element', name: 'テストアイテム10', cost: 1, vp: 1, effect: '', sets: ['A', 'D'] },
    { id: 'item_piercing', name: 'テストアイテム11', cost: 1, vp: 1, effect: '', sets: ['B'] },
    { id: 'item_protect', name: 'テストアイテム12', cost: 1, vp: 1, effect: '', sets: ['B'] },
    { id: 'item_reactivate', name: 'テストアイテム13', cost: 1, vp: 1, effect: '', sets: ['C'] },
    { id: 'item_selfbounce', name: 'テストアイテム14', cost: 1, vp: 1, effect: '', sets: ['D'] },
  ],
};
...
describe('STANDARD_BOARD_ATTRS', () => {
  ...
  it('拳×2, 念×2, 光×2, 闇×2, 虚×1', () => {
    const counts = STANDARD_BOARD_ATTRS.reduce<Record<string, number>>((acc, a) => {
      acc[a] = (acc[a] ?? 0) + 1;
      return acc;
    }, {});
...
describe('createInitialGameState', () => {
  ...
  const draft = completeDraft(['aggro', 'tank'], 'A', ['control', 'synergy'], 'B');

  it('盤面は全マス null', () => {
    const state = createInitialGameState(draft, MOCK_DB, { boardAttrs: FIXED_ATTRS });
  ...
  it('firstPlayer オプションで先手を指定できる', () => {
    const state = createInitialGameState(draft, MOCK_DB, { boardAttrs: FIXED_ATTRS, firstPlayer: 1 });
