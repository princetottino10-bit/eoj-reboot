...
// ============================================================
describe('定数', () => {
  it('VP勝利閾値 = 15', () => { expect(VP_WIN_THRESHOLD).toBe(15); });
  it('領土勝利マス数 = 5', () => { expect(TERRITORY_WIN_COUNT).toBe(5); });
  it('タイムアウトターン = 50', () => { expect(TIMEOUT_TURN).toBe(50); });
});
...
describe('checkVPWin', () => {
  ...
  it('両方15VP → 0を返す（P0優先）', () => {
    expect(checkVPWin([makePlayer({ vp: 15 }), makePlayer({ vp: 15 })])).toBe(0);
  });
  it('両方14VP → null', () => {
    expect(checkVPWin([makePlayer({ vp: 14 }), makePlayer({ vp: 14 })])).toBeNull();
  });
  it('0VP同士 → null', () => {
...
describe('checkTerritoryWin', () => {
  ...
  it('P0とP1が混在してもそれぞれカウント', () => {
    const board = [
      makeChar(0), makeChar(0), makeChar(0), makeChar(0), makeChar(0),
      makeChar(1), makeChar(1), makeChar(1), null,
    ];
    expect(checkTerritoryWin(board, 0)).toBe(0);
...
describe('checkTimeoutWin', () => {
  it('49ターン → null（まだ判定しない）', () => {
    expect(checkTimeoutWin(49, [makePlayer({ vp: 10 }), makePlayer({ vp: 0 })])).toBeNull();
  });
  it('50ターン、P0 VP多 → 0', () => {
    expect(checkTimeoutWin(50, [makePlayer({ vp: 10 }), makePlayer({ vp: 5 })])).toBe(0);
  });
  it('50ターン、P1 VP多 → 1', () => {
    expect(checkTimeoutWin(50, [makePlayer({ vp: 3 }), makePlayer({ vp: 8 })])).toBe(1);
  });
  it('50ターン、同VP → -1（引き分け）', () => {
    expect(checkTimeoutWin(50, [makePlayer({ vp: 5 }), makePlayer({ vp: 5 })])).toBe(-1);
  });
  it('50ターン超でも同様に判定', () => {
    expect(checkTimeoutWin(99, [makePlayer({ vp: 10 }), makePlayer({ vp: 5 })])).toBe(0);
  });
  it('0VP同士で50ターン → 引き分け', () => {
...
describe('evalVictory', () => {
  ...
  it('VP勝利が最優先（領土・タイムアウトより先）', () => {
    const board = [...Array(5).fill(makeChar(0)), ...Array(4).fill(null)];
    const state = makeState({ players: [makePlayer({ vp: 15 }), makePlayer()], board });
