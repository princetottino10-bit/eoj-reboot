...
import { applyPendingEffect, applyDiscardEffect } from '../src/engine/pendingEffects.js';
...
describe('applyPendingEffect: control_v2_10 ATK-1+条件ドロー・ダメ', () => {
  it('ATK-1のみ（デバフ0体）', () => {
    ...
    const state = makeState(board, { players: [makePlayer({ deck: ['c1'], hand: [] }), makePlayer()] });
  ...
  it('デバフ1体以上でドロー', () => {
    ...
    const state = makeState(board, { players: [makePlayer({ deck: ['c1', 'c2'], hand: [] }), makePlayer()] });
    const result = applyPendingEffect(state, 'control_v2_10', 4, 3, 0);
...
describe('applyDiscardEffect: snipe_v2_07 2ドロー', () => {
  it('捨て時に2ドロー', () => {
    const board: Board = Array(9).fill(null);
    const state = makeState(board, {
      players: [makePlayer({ deck: ['c1', 'c2', 'c3'], hand: [] }), makePlayer()],
    });
