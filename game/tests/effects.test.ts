...
import type { Board, CharInstance, PlayerState, GameState } from '../src/engine/types.js';
...
function makeState(board: Board, players?: [PlayerState, PlayerState]): GameState {
  return {
    turn: 0, active: 0,
...
describe('getSummonEffect', () => {
  ...
  it('tank_v2_02 のclauses: give_marker adj_allies protection', () => {
    ...
    expect(clauses[0]?.effects[0]).toMatchObject({ type: 'give_marker', target: 'adj_allies', marker: 'protection' });
  });

  it('synergy_v2_07 のclauses: atk_delta adj_allies +1', () => {
    const { clauses } = getSummonEffect('synergy_v2_07');
    expect(clauses[0]?.effects[0]).toMatchObject({ type: 'atk_delta', target: 'adj_allies', delta: 1 });
  });
});
...
describe('applyAutoEffects: give_marker adj_allies', () => {
  const fx: EffectClause[] = [{ trigger: 'on_summon', effects: [{ type: 'give_marker', target: 'adj_allies', marker: 'protection' }] }];
...
describe('applyAutoEffects: give_marker self_and_adj_allies', () => {
  const fx: EffectClause[] = [{ trigger: 'on_summon', effects: [{ type: 'give_marker', target: 'self_and_adj_allies', marker: 'protection' }] }];
...
describe('applyAutoEffects: give_marker self', () => {
  const fx: EffectClause[] = [{ trigger: 'on_summon', effects: [{ type: 'give_marker', target: 'self', marker: 'protection' }] }];
...
describe('applyAutoEffects: heal adj_allies', () => {
  const fx: EffectClause[] = [{ trigger: 'on_summon', effects: [{ type: 'heal', target: 'adj_allies', amount: 2 }] }];
...
describe('applyAutoEffects: atk_delta adj_allies', () => {
  const fxPlus: EffectClause[] = [{ trigger: 'on_summon', effects: [{ type: 'atk_delta', target: 'adj_allies', delta: 1 }] }];
  const fxMinus: EffectClause[] = [{ trigger: 'on_summon', effects: [{ type: 'atk_delta', target: 'adj_allies', delta: -1 }] }];
...
describe('applyAutoEffects: hp_delta adj_enemies', () => {
  const fx: EffectClause[] = [{ trigger: 'on_summon', effects: [{ type: 'hp_delta', target: 'adj_enemies', amount: -3 }] }];
...
describe('applyAutoEffects: mana_steal', () => {
  const fx: EffectClause[] = [{ trigger: 'on_summon', effects: [{ type: 'mana_steal', amount: 2 }] }];
  ...
  it('相手から 2 マナ奪う（十分あれば）', () => {
    ...
    const { players: np } = applyAutoEffects(makeState(board, players), 4, 0, fx);
  ...
  it('P1がsummonerの場合はP0から奪う', () => {
    ...
    const { players: np } = applyAutoEffects(makeState(board, players), 4, 1, fx);
    expect(np[1].mana).toBe(3);
...
describe('applyAutoEffects: 複数 clause・複数 effects', () => {
  ...
  it('複数の独立 clause を順番に適用', () => {
    const fx: EffectClause[] = [
      ...
      { trigger: 'on_summon', effects: [{ type: 'give_marker', target: 'self', marker: 'protection' }] },
    ];
    const board = emptyBoard();
...
describe('applyAutoEffects: フィルタリング', () => {
  it('select_* target を持つ clause はスキップされる', () => {
    const fx: EffectClause[] = [{
      trigger: 'on_summon',
      effects: [{ type: 'give_marker', target: 'select_ally', marker: 'protection' }],
    }];
    const board = emptyBoard();
    board[4] = makeChar(0);
    board[1] = makeChar(0);
  ...
  it('discard cost を持つ clause はスキップされる', () => {
    const fx: EffectClause[] = [{
      ...
      effects: [{ type: 'give_marker', target: 'adj_allies', marker: 'protection' }],
    }];
    const board = emptyBoard();
    board[4] = makeChar(0);
    board[1] = makeChar(0);
  ...
  it('on_summon 以外のトリガーはスキップされる', () => {
    const fx: EffectClause[] = [
      { trigger: 'on_attack', effects: [{ type: 'give_marker', target: 'adj_allies', marker: 'protection' }] },
      { trigger: 'passive', effects: [{ type: 'give_marker', target: 'adj_allies', marker: 'evasion' }] },
    ];
    const board = emptyBoard();
    board[4] = makeChar(0);
    board[1] = makeChar(0);
