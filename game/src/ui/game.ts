...
export interface GameUiExtra {
  ...
  onAttackContext: {
    ...
    postAttackSelfRotate180: boolean;
    postAttackSelfBounce: boolean;
    postAttackSkipNextTurn: boolean;
    postAttackAtkDeltaTarget: number;
    postAttackPushTarget: number;
    postAttackGiveMarkerSelf: MarkerKey | null;
  } | null;
}
    ...
    setState({
      ...
      gameUiExtra: {
        ...
        onAttackContext: {
          ...
          postAttackSelfRotate180: false,
          postAttackSelfBounce: false,
          postAttackSkipNextTurn: false,
          postAttackAtkDeltaTarget: 0,
          postAttackPushTarget: 0,
          postAttackGiveMarkerSelf: null,
        },
...
interface PostAttackEffects {
  selfRotate180: boolean;
  selfBounce: boolean;
  skipNextTurn: boolean;
  atkDeltaTarget: number;
  pushTarget: number;
  giveMarkerSelf: MarkerKey | null;
}
...
function executeAttack(
  ...
  setPiercing: boolean,
  rotateAfterAttack: boolean,
  postAttack: PostAttackEffects | null = null,
): void {
  const active = state.active;
  const attacker = state.board[attackerIdx];
  if (!attacker) {
  ...
  // Apply post-attack effects from optional cost clause
  if (postAttack) {
    if (postAttack.giveMarkerSelf && newState.board[attackerIdx] != null) {
      const nb = [...newState.board] as Board;
      // biome-ignore lint/style/noNonNullAssertion: null check above
      const a = nb[attackerIdx]!;
      const mk = postAttack.giveMarkerSelf;
      nb[attackerIdx] = { ...a, markers: { ...a.markers, [mk]: a.markers[mk] + 1 } };
      newState = appendLog({ ...newState, board: nb }, `攻撃時効果: 自身に${mk}マーカー付与`, "info");
    }
    if (postAttack.atkDeltaTarget !== 0 && newState.board[targetIdx] != null) {
      const nb = [...newState.board] as Board;
      // biome-ignore lint/style/noNonNullAssertion: null check above
      const t = nb[targetIdx]!;
      const newAtk = Math.max(0, t.atk + postAttack.atkDeltaTarget);
      nb[targetIdx] = { ...t, atk: newAtk, baseAtk: newAtk };
      const sign = postAttack.atkDeltaTarget > 0 ? "+" : "";
      newState = appendLog({ ...newState, board: nb }, `攻撃時効果: 対象ATK${sign}${postAttack.atkDeltaTarget}（永続）`, "info");
    }
    if (postAttack.pushTarget > 0 && newState.board[targetIdx] != null) {
      let nb = newState.board;
      for (let i = 0; i < postAttack.pushTarget; i++) {
        const pushed = pushBack(nb, targetIdx);
        if (!pushed) break;
        nb = pushed;
      }
      newState = appendLog({ ...newState, board: nb }, `攻撃時効果: 対象を${postAttack.pushTarget}マス押し出し`, "info");
    }
    if (postAttack.selfRotate180 && newState.board[attackerIdx] != null) {
      const nb = [...newState.board] as Board;
      // biome-ignore lint/style/noNonNullAssertion: null check above
      const a = nb[attackerIdx]!;
      nb[attackerIdx] = { ...a, dir: ((a.dir + 2) % 4) as Direction };
      newState = appendLog({ ...newState, board: nb }, `攻撃時効果: 自身が180°回転（露出）`, "info");
    }
    if (postAttack.skipNextTurn && newState.board[attackerIdx] != null) {
      const nb = [...newState.board] as Board;
      // biome-ignore lint/style/noNonNullAssertion: null check above
      const a = nb[attackerIdx]!;
      nb[attackerIdx] = { ...a, status: { ...a.status, skipNextTurn: a.status.skipNextTurn + 1 } };
      newState = appendLog({ ...newState, board: nb }, `攻撃時効果: 次のターン行動不可`, "info");
    }
    if (postAttack.selfBounce && newState.board[attackerIdx] != null) {
      const nb = [...newState.board] as Board;
      // biome-ignore lint/style/noNonNullAssertion: null check above
      const a = nb[attackerIdx]!;
      nb[attackerIdx] = null;
      const np = [...newState.players] as typeof newState.players;
      np[active] = { ...np[active], hand: [...np[active].hand, a.cardId] };
      newState = appendLog({ ...newState, board: nb, players: np }, `攻撃時効果: ${getCardName(a.cardId)} を手札に戻した`, "info");
    }
  }
...
function buildActionPanel(state: GameState, ui: GameUiExtra): HTMLElement {
  ...
  if (ui.mode === "summon_done") {
        ...
        if (optClause?.cost) {
                    ...
                    : c.type === "vp"
                      ...
                      : c.type === "mill"
                        ...
                        : c.type === "expose_self"
                          ...
                          : c.type === "spend_marker"
                            ? "自身のマーカー1枚消費"
                            : c.type === "skip_next_turn"
                              ? "次のターン行動不可"
                              : c.type === "self_bounce"
                                ? "自身を手札に戻す"
                                : c.type === "sacrifice_ally"
                                  ? "味方1体を生贄"
                                  : "";
          const effectDesc = optClause.effects
            .map((e) => {
              ...
              if (e.type === "mana_gain") return `マナ+${e.amount}`;
              if (e.type === "push" && (e as { target: string }).target === "attack_target")
                return `対象を${e.distance}マス押し出し`;
              if (e.type === "atk_delta" && (e as { target: string }).target === "attack_target")
                return `対象ATK${e.delta > 0 ? "+" : ""}${e.delta}（永続）`;
              if (e.type === "give_marker" && (e as { target: string }).target === "self")
                return `自身に${(e as { marker: string }).marker}マーカー`;
              if (e.type === "rotate" && (e as { target: string }).target === "attack_target")
                return "対象の向きを変更";
              return "";
            })
            .filter(Boolean)
          ...
          const canAfford = (() => {
            ...
            if (c.type === "vp")
              return state.players[active].vp >= c.amount;
            if (c.type === "spend_marker") {
              const a = attacker;
              return (
                a.markers.protection > 0 ||
                a.markers.evasion > 0 ||
                a.markers.piercing > 0 ||
                a.markers.quickness > 0 ||
                a.markers.aim > 0
              );
            }
            if (c.type === "sacrifice_ally") {
    ...
    actionPanel.appendChild(hint);
  } else if (ui.mode === "on_attack_sacrifice_pending") {
    const hint = document.createElement("div");
    hint.className = "action-label";
    hint.style.width = "100%";
    hint.textContent = "生贄にする味方を1体選択（攻撃時コスト）";
    actionPanel.appendChild(hint);
...
function buildPostAttack(ctx: NonNullable<GameUiExtra["onAttackContext"]>): PostAttackEffects {
  return {
    selfRotate180: ctx.postAttackSelfRotate180,
    selfBounce: ctx.postAttackSelfBounce,
    skipNextTurn: ctx.postAttackSkipNextTurn,
    atkDeltaTarget: ctx.postAttackAtkDeltaTarget,
    pushTarget: ctx.postAttackPushTarget,
    giveMarkerSelf: ctx.postAttackGiveMarkerSelf,
  };
}
...
function onOnAttackCostYes(state: GameState, ui: GameUiExtra): void {
  ...
  let rotateAfterAttack = ctx.rotateAfterAttack;
  let postAtkDelta = ctx.postAttackAtkDeltaTarget;
  let postPushTarget = ctx.postAttackPushTarget;
  let giveMarkerSelf: MarkerKey | null = ctx.postAttackGiveMarkerSelf;
  ...
  if (condOk) {
    for (const eff of optClause.effects) {
      if (eff.type === "extra_damage") extraDamage += eff.amount;
        ...
        postAtkDelta += (eff as { delta: number }).delta;
      else if (
        eff.type === "push" &&
        (eff as { target: string }).target === "attack_target"
      )
        postPushTarget += (eff as { distance: number }).distance;
      else if (
        eff.type === "give_marker" &&
        (eff as { target: string }).target === "self"
      )
        giveMarkerSelf = (eff as { marker: MarkerKey }).marker;
    }
  }
  ...
  const updCtx = {
    ...
    postAttackAtkDeltaTarget: postAtkDelta,
    postAttackPushTarget: postPushTarget,
    postAttackGiveMarkerSelf: giveMarkerSelf,
  };
  ...
  if (cost.type === "self_damage") {
    ...
    executeAttack(ns, attackerIdx, targetIdx, isSummonAttack, updCtx.extraDamage, updCtx.setPiercing, updCtx.rotateAfterAttack, buildPostAttack(updCtx));
  } else if (cost.type === "mana") {
    if (state.players[active].mana < cost.amount) {
      executeAttack(state, attackerIdx, targetIdx, isSummonAttack, ctx.extraDamage, ctx.setPiercing, ctx.rotateAfterAttack);
      return;
    }
    const np = [...state.players] as typeof state.players;
    ...
    if (ns.players[active].hand.length === 0) {
      executeAttack(ns, attackerIdx, targetIdx, isSummonAttack, ctx.extraDamage, ctx.setPiercing, ctx.rotateAfterAttack);
      return;
    }
    setState({
      gameState: ns,
  ...
  } else if (cost.type === "vp") {
    if (state.players[active].vp < cost.amount) {
      executeAttack(state, attackerIdx, targetIdx, isSummonAttack, ctx.extraDamage, ctx.setPiercing, ctx.rotateAfterAttack);
      return;
    }
    const np = [...state.players] as typeof state.players;
    np[active] = { ...np[active], vp: np[active].vp - cost.amount };
    const ns = appendLog(
      { ...state, players: np },
      `攻撃時コスト: VP${cost.amount}消費`,
      "info",
    );
    executeAttack(ns, attackerIdx, targetIdx, isSummonAttack, updCtx.extraDamage, updCtx.setPiercing, updCtx.rotateAfterAttack, buildPostAttack(updCtx));
  } else if (cost.type === "mill") {
    const deck = [...state.players[active].deck];
    const discard = [...state.players[active].discard];
    const milled: string[] = [];
    for (let i = 0; i < cost.count && deck.length > 0; i++) {
      // biome-ignore lint/style/noNonNullAssertion: deck.length > 0 checked
      milled.push(deck.pop()!);
    }
    discard.push(...milled);
    const np = [...state.players] as typeof state.players;
    np[active] = { ...np[active], deck, discard };
    const ns = appendLog(
      { ...state, players: np },
      `攻撃時コスト: デッキトップ${milled.length}枚を捨て札へ`,
      "info",
    );
    // mana_gain reward is applied immediately
    let ns2 = ns;
    if (condOk) {
      for (const eff of optClause.effects) {
        if (eff.type === "mana_gain") {
          const np2 = [...ns2.players] as typeof ns2.players;
          np2[active] = { ...np2[active], mana: np2[active].mana + eff.amount };
          ns2 = appendLog({ ...ns2, players: np2 }, `攻撃時効果: マナ+${eff.amount}`, "info");
        }
      }
    }
    executeAttack(ns2, attackerIdx, targetIdx, isSummonAttack, updCtx.extraDamage, updCtx.setPiercing, updCtx.rotateAfterAttack, buildPostAttack(updCtx));
  } else if (cost.type === "spend_marker") {
    const nb = [...state.board] as Board;
    const a = nb[attackerIdx];
    let ns = state;
    if (a) {
      const mk = (["piercing", "protection", "evasion", "quickness", "aim"] as MarkerKey[]).find(
        (k) => a.markers[k] > 0,
      );
      if (mk) {
        nb[attackerIdx] = { ...a, markers: { ...a.markers, [mk]: a.markers[mk] - 1 } };
        ns = appendLog({ ...state, board: nb }, `攻撃時コスト: ${mk}マーカーを1枚消費`, "info");
      }
    }
    executeAttack(ns, attackerIdx, targetIdx, isSummonAttack, updCtx.extraDamage, updCtx.setPiercing, updCtx.rotateAfterAttack, buildPostAttack(updCtx));
  } else if (cost.type === "expose_self") {
    const updCtx2 = { ...updCtx, postAttackSelfRotate180: true };
    executeAttack(state, attackerIdx, targetIdx, isSummonAttack, updCtx2.extraDamage, updCtx2.setPiercing, updCtx2.rotateAfterAttack, buildPostAttack(updCtx2));
  } else if (cost.type === "skip_next_turn") {
    const updCtx2 = { ...updCtx, postAttackSkipNextTurn: true };
    executeAttack(state, attackerIdx, targetIdx, isSummonAttack, updCtx2.extraDamage, updCtx2.setPiercing, updCtx2.rotateAfterAttack, buildPostAttack(updCtx2));
  } else if (cost.type === "self_bounce") {
    const updCtx2 = { ...updCtx, postAttackSelfBounce: true };
    executeAttack(state, attackerIdx, targetIdx, isSummonAttack, updCtx2.extraDamage, updCtx2.setPiercing, updCtx2.rotateAfterAttack, buildPostAttack(updCtx2));
  } else if (cost.type === "sacrifice_ally") {
    const allyIdxs = state.board
      .map((ch, i) => (ch !== null && ch.owner === active && i !== attackerIdx ? i : -1))
      .filter((i) => i >= 0) as CellIndex[];
    ...
    setState({
      ...
      gameUiExtra: {
        ...resetGameUiExtra(),
        mode: "on_attack_sacrifice_pending",
        validCells: allyIdxs,
        onAttackContext: updCtx,
        magicAttackContext: ui.magicAttackContext,
      },
...
function onOnAttackSacrificePicked(
  state: GameState,
  ui: GameUiExtra,
  sacrificeIdx: CellIndex,
): void {
  const ctx = ui.onAttackContext;
  if (!ctx) return;
  const active = state.active;
  const sacrificed = state.board[sacrificeIdx];
  if (!sacrificed || sacrificed.owner !== active || sacrificeIdx === ctx.attackerIdx) return;

  const nb = [...state.board] as Board;
  nb[sacrificeIdx] = null;
  const np = [...state.players] as typeof state.players;
  np[active] = { ...np[active], discard: [...np[active].discard, sacrificed.cardId] };
  const ns = appendLog(
    { ...state, board: nb, players: np },
    `攻撃時コスト: ${getCardName(sacrificed.cardId)} を生贄に捧げた`,
    "info",
  );
  executeAttack(
    ns,
