import { getCardName, getCharDef } from "../data/cards.js";
import { getAdjacentCells, getFrontRowCells } from "./board.js";
import { clearAffiliatedEffects } from "./combat.js";
import type { Board, CellIndex, GameState } from "./types.js";
import { appendLog } from "./types.js";

export function applyUltDirectEffect(
  state: GameState,
  casterIdx: CellIndex,
  active: 0 | 1,
): GameState {
  ...
  switch (cardId) {
    case "aggro_v2_09": {
      nb[casterIdx] = {
        ...caster,
        markers: { ...caster.markers, piercing: caster.markers.piercing + 1 },
      };
      return appendLog(
        { ...state, board: nb },
        "貫通マーカーを1枚獲得",
      ...
      for (const ci of frontCells) {
        ...
        if (target && target.owner === opp) {
          ...
          if (newHp <= 0) {
            ...
            np[active] = {
              ...np[active],
              vp: np[active].vp + (getCharDef(target.cardId)?.vp ?? 1),
            };
          } else {
      ...
      for (const ai of adjIdxs) {
        ...
        if (ally && ally.owner === active) {
          nb[ai] = {
            ...
            markers: {
              ...ally.markers,
              protection: ally.markers.protection + 1,
            },
          };
        }
      }
    ...
    case "control_v2_10": {
      for (let i = 0; i < 9; i++) {
        ...
        if (c && c.owner === opp)
          nb[i] = {
            ...c,
            atk: Math.max(0, c.atk - 1),
            baseAtk: Math.max(0, c.baseAtk - 1),
          };
      }
      ...
      for (let i = 0; i < 9; i++) {
        ...
        if (c && c.owner === active) {
          nb[i] = {
            ...c,
            markers: {
              protection: c.markers.protection + 1,
              evasion: c.markers.evasion + 1,
              piercing: c.markers.piercing + 1,
              quickness: c.markers.quickness + 1,
              aim: c.markers.aim,
            },
          };
        }
    ...
    case "trick_v2_12": {
      nb[casterIdx] = {
        ...caster,
        status: { ...caster.status, immune: caster.status.immune + 1 },
      };
  ...
  switch (casterCardId) {
    ...
    case "snipe_v2_12": {
      ...
      if (target && target.owner === opp) {
        ...
        if (newHp <= 0) {
          ...
          np2[active] = {
            ...np2[active],
            vp: np2[active].vp + (getCharDef(target.cardId)?.vp ?? 1),
          };
          newState = appendLog(
            { ...newState, board: nb, players: np2 },
            `${getCardName(target.cardId)} に${dmg}ダメ（貫通）撃破！VP獲得`,
            "system",
          );
        } else {
          ...
          newState = appendLog(
            { ...newState, board: nb },
            `${getCardName(target.cardId)} に${dmg}ダメ（貫通）`,
            "damage",
          );
        }
      }
    ...
    case "control_v2_09": {
      ...
      if (target && target.owner === opp && casterChar) {
        ...
        nb[targetIdx] = {
          ...target,
          atk: Math.max(0, target.atk - steal),
          baseAtk: Math.max(0, target.baseAtk - steal),
        };
        nb[casterIdx] = {
          ...casterChar,
          atk: casterChar.atk + steal,
          baseAtk: casterChar.baseAtk + steal,
        };
        newState = appendLog(
          { ...newState, board: nb },
          `${getCardName(target.cardId)} のATKを${steal}奪った`,
          "info",
        );
      }
      return { newState, continueToDir: false };
    ...
    case "trick_v2_09": {
      ...
      newState = appendLog(
        { ...newState, board: nb },
        `${getCardName(casterChar?.cardId ?? "")} と ${getCardName(targetChar?.cardId ?? "")} の位置を入れ替えた`,
        "info",
      );
