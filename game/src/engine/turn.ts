...
export function startTurnPhase(state: GameState): GameState {
  ...
  const newBoard = state.board.map((char) => {
    ...
    const skipNextTurn = s.skipNextTurn > 0 ? s.skipNextTurn - 1 : 0;
    const newStatus = {
      ...s,
      brainwashedTurns,
    ...
    const forcedActed = s.skipNextTurn > 0;
    return { ...char, status: newStatus, hasActed: forcedActed, hasRotated: forcedActed };
  });
