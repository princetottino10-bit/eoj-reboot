...
import { getAdjacentCells, cellRow, cellCol, isValidCell, cellIdx, relToAbs } from './board.js';
 ...
 */
export function calcCostReduction(cardDef: CharCardDef, board: Board, active: 0 | 1): number {
  ...
  switch (cardDef.faction) {
    case 'aggro': {
      ...
      return board.filter(c => c !== null && c.owner === active && c.hasActed === true).length;
    }
    case 'tank': {
      // Number of active player's chars with 'カバー' keyword that have at least 1 adjacent ally
      return board.filter((c, idx) => {
        if (c === null || c.owner !== active || !c.keywords.includes('カバー')) return false;
    ...
    case 'control': {
      ...
      return board.filter(c => {
        ...
        return c.atk < c.baseAtk || c.status.actionTax > 0 || c.status.brainwashedTurns > 0;
      }).length;
    }
    case 'synergy': {
      ...
      return board.filter(c => {
        ...
        return (
          c.markers.protection + c.markers.evasion + c.markers.piercing +
          c.markers.quickness + c.markers.aim > 0
        );
      }).length;
    }
    case 'snipe': {
      // Number of opponent's chars with aim markers > 0 (currently always 0)
      return board.filter(c => c !== null && c.owner === opp && c.markers.aim > 0).length;
    }
    case 'trick': {
